...
// Модуль высокоуровневых процедур работы с файлами
 ...
 * - Файл может существовать, а может и отсутствовать (в таком случае он будет создан)
 * - Если файл существует, то формат файла должен быть корректен,
 * в противном случае, файл будет очищен и в него будут записаны новые данные.
 ...
 * - Количество элементов данных в массиве может быть больше
 * максимального количества элементов в файле (в таком случае,
 * записаные данные будут перезатёрты данными из того же самого
 * массива).
 ...
 * соответствует хронологически наиболее позднему записанному значению.
 * Так как файл может быть не полон, то процедура может вернуть меньшее
 ...
 * \param filePath - путь и имя файла.
 * \param db - указатель на целевой массив данных, элемент с индексом 0,
 *              соответствует самому позднему записанному значению в файл.
 *              Количество прочитанных данных содержится в памяти,
 *              на которую ссылается указатель readCount.
 * \param count - количество запрашиваемых элементов данных.
 * \param offset - смещение позиции от последнего записанного эелемнта в файле.
 *                 Если offset = 0, то чтение данных производится с
 *                 последнего записанного элемента в файл.
 *                 Если offset больше чем доступно элементов в файле,
 *                 то не будет прочитано ничего.
